    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<ApiResponse<OrderDto>> CreateOrderAsync(int userId, CreateOrderDto dto)
        {
            ...
            var order = new Order
            {
                ...
                PaymentStatus = dto.PaymentMethod == PaymentMethods.CashOnDelivery
                                    ? PaymentStatus.Pending
                                    : PaymentStatus.Completed,
                OrderStatus = dto.PaymentMethod == PaymentMethods.CashOnDelivery
                                    ? OrderStatus.Processing
                                    : OrderStatus.Pending,
                TotalAmount = totalAmount,
                CreatedOn = DateTime.UtcNow,
                Items = cart.Items.Select(c => new OrderItems
                {
        ...
        public async Task<OrderDto> UpdateOrderStatusAsync(int orderId, OrderStatus newStatus)
        {
            ...
            if (order.PaymentMethod == PaymentMethods.CashOnDelivery && newStatus == OrderStatus.Delivered)
